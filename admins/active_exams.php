...
/*------  fetching active exams from databaase ------*/
$exams = $conn->query("
    SELECT * FROM exams
...
<head>
    ...
    <link rel="stylesheet" href="assest/css/active_exams.css">
</head>
...
<body>
    ...
    <div class="main">
        <h4 class="mb-3">
            Active Exams <span class="badge badge-active ms-2">LIVE</span>
        </h4>
        ...
        <?php if ($exams->num_rows == 0): ?>
            <div class="alert alert-warning">No active exams right now.</div>
        <?php endif; ?>
        ...
        <?php while ($row = $exams->fetch_assoc()): ?>
            ...
            <?php
            $qCount = $conn->query(
                "SELECT COUNT(*) total FROM exam_questions WHERE exam_id = {$row['id']}"
            )->fetch_assoc()['total'];
            ?>
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="exam_name" value="<?= htmlspecialchars($row['exam_name']) ?>" class="form-control">
                            </div>

                            <div class="col-md-2">
                                <label class="form-label">Start Date</label>
                                <input type="date" name="start_date" value="<?= substr($row['start_date'], 0, 10) ?>" class="form-control">
                            </div>

                            <div class="col-md-2">
                                <label class="form-label">End Date</label>
                                <input type="date" name="end_date" value="<?= substr($row['end_date'], 0, 10) ?>" class="form-control">
                            </div>

                            <div class="col-md-2">
                                <label class="form-label">Attempts</label>
                                <input type="number" name="attempts" value="<?= $row['attempts'] ?>" class="form-control">
                            </div>

                            <div class="col-md-1">
                                <label class="form-label">Duration</label>
                                <input type="number" name="duration_minutes" value="<?= $row['duration_minutes'] ?>" class="form-control">
                            </div>

                            <div class="col-md-2">
                                <label class="form-label">Questions</label>
                                <input type="number" value="<?= $qCount ?>" class="form-control" readonly>
                            </div>
                        </div>
                        ...
                        <div class="mt-3 d-flex gap-2 flex-wrap">
                            ...
                            <a href="edit_questions.php?exam_id=<?= $row['id'] ?>" class="btn btn-primary btn-sm">
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            Delete <b><?= htmlspecialchars($row['exam_name']) ?></b>?<br>
                            <small class="text-danger">All questions will be removed.</small>
                        </div>
